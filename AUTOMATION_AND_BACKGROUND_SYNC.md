...
- A Personal Automation triggered when HugoInk opens is the recommended auto-pull setup. A manual Shortcut while the device is unlocked is the most reliable option.
...
HugoInk intentionally does not register `BGAppRefreshTask` or `BGProcessingTask` today:
...
If a Shortcut reports that protected data or an external folder is unavailable, unlock the device, open HugoInk, restore folder access if requested, and run the Shortcut again.
